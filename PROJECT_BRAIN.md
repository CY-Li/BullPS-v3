...
**2026-02-10 架構重整公告：**
專案已轉型為純美股架構 (US-Quant)，移除所有台股代碼。新架構位於根目錄：
- `core/`: 分析邏輯 (integrated_stock_analyzer.py, bps_optimizer.py)
- `tools/`: 執行工具 (scan.py, profit_monitor.py)
- `config/`: 設定檔
- `backend/` & `frontend/`: Web 介面

---

## 1. 核心分析引擎 (`core/integrated_stock_analyzer.py`)
- **進場分數 (Entry Score)**: 
...
## 4. 監控與停利 (`tools/profit_monitor.py`)
...
## 5. 操作原則 (Rules for AI)
1. **路徑變更**: 腳本已移至根目錄 `tools/`，請使用 `python3 tools/scan.py`。
2. **優先查表**: 針對持倉的建議，必須先讀取 `monitored_stocks.json`。
