...
class IntegratedStockAnalyzer:
    ...
    def calculate_technical_indicators(self, data):
        ...
        # 新增: Long Signal Price (趨勢啟動價)
        df['Long_Signal_Price'] = self.calculate_long_signal_price(df)
    ...
    def analyze_stock(self, symbol):
        ...
        try:
            ...
            # [V4 Upgrade] 新增指標
            long_days = df['Uptrend_Continuity'].iloc[-1] if 'Uptrend_Continuity' in df else 0
            long_signal_price = df['Long_Signal_Price'].iloc[-1] if 'Long_Signal_Price' in df else 0
    ...
    def calculate_uptrend_continuity(self, df):
        ...
        try:
            ...
            close = df['Close']
            above_ma = close > ma20
            groups = (above_ma != above_ma.shift()).cumsum()
            streak = above_ma.groupby(groups).cumsum()
            return streak.where(above_ma, 0)
        except:
            return pd.Series(0, index=df.index)

    def calculate_long_signal_price(self, df):
        """
        找出 Long Signal Price (趨勢啟動價格)
        定義: 最近一次突破 MA20 的當天收盤價
        """
        try:
            ma20 = df['MA20']
            close = df['Close']
            crossover = (close > ma20) & (close.shift(1) <= ma20.shift(1))
            signal_prices = close.where(crossover, np.nan)
            return signal_prices.ffill().fillna(0)
        except:
            return pd.Series(0, index=df.index)
